        ...
        <div class="content">
            <div class="card mt-3">
                <div class="card-header" style="background:#A0DAA9;">
                    <h1 class="d-inline-block card-title">Manifest Received&nbsp;&nbsp;</h1>
                    ...
                    <div class="col-md-2.1 float-right">
                        <a href="<?php echo base_url();?>Branch/ManifestReceived/masterManifestReceived" class="btn btn-sm btn-secondary"  style="margin-left: 1em;color:white;">Master view</a>
                    </div>
                </div>
            </div>
            ...
            <section class="content" style="background: RGB(273, 267, 240);">
                <h4 class="d-inline-block mb-5" style="text-color:gray;">STORE RECEIVED MANIFEST DATA ACCORDING TO CN NUMBER&nbsp;&nbsp;</h4>
                <?php
                    $MFNo = 10000;
                    if($manifest_number){
                ...
                ?>
                <form method="post" id="manifestReceivedForm" action="<?php echo base_url(); ?>Branch/ManifestReceived/saveReceivedManifest">
                    <div class="form-row">
                        ...
                        <div class="col-md-3 mb-5">
                            <label for="validationDefault04">Sender Branch/Hub</label>
                            <select name="sender_branch" id="branch" class="form-control" required>
                ...
                <div style="margin-top: 5px;">
                    <input type="button" name="manifest" id="manifestReceived" class="btn btn-primary" value="SAVE MANIFEST" />  
                </div>
            </section>
...
<script>
    ...
    if(!branch){
       //$.alert.open('Choose Receiver Hub !');
       alert("Choose Sender Hub ! ");
       $("#cnn").val("");
       $("#branch").focus();
       return false;
    }   
        var url = "<?php echo base_url(); ?>Branch/ManifestReceived/getRequiredManifest";
        $.ajax({
            type: "POST",
            url: url,
            async: true,
            data: {cn : cnn, sender_branch : branch},
            dataType: "html",
            success : function(data) {
                    // alert('success');
                    if(data == 0){
                            alert("CONSIGNMENT NUMBER NOT MATCHED ACCORDING TO SENDER BRANCH !!");
                            return false;
                        }
                        scanheadFlag = 1;
                        if(scanheadFlag == 1){
...
$("#manifestReceived").click(function(){
    if(scanheadFlag == 1) {
        $("#manifestReceivedForm").submit();
    } else {
